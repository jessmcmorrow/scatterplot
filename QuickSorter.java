...
/**
   ...
   @param to the last index of the portion to be sorted
*/
public static void sortDouble(double[] a, int from, int to)
{
   if (from >= to) { return; }
   int p = partitionDouble(a, from, to);
   sortDouble(a, from, p);
   sortDouble(a, p + 1, to);
}
...
/**
   ...
   @return the last index of the first partition
*/
private static int partitionDouble(double[] a, int from, int to)
{
	double pivot = a[from];
	int i = from - 1;
	int j = to + 1;
